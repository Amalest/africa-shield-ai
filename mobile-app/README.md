...
| SMS alert channel toggle | **Real** — `SubscriberService` calls `POST /api/subscribers` (subscribe) / `DELETE /api/subscribers/{phone_number}` (unsubscribe) for real, using the phone number entered at onboarding (Settings > Location). **As of 2026-09-07, both directions require verifying phone-number ownership first** — toggling either way requests a one-time code (`POST /api/subscribers/verify/request`) and prompts for it in a dialog before completing; the backend used to accept either with no proof at all, which for *unsubscribing* someone from real flood warnings was a real safety gap. When SMS isn't configured server-side, the code comes back directly for testing (shown in the dialog with a "for testing only" note) instead of requiring a real phone. |
